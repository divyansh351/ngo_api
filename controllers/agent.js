...
module.exports.registerAgent = async (req, res) => {
    try {
        ...
        console.log("HELLO2")
        agent.agent_photo = req.files.map(f => ({ url: f.path, filename: f.filename }));
...
module.exports.loginAgent = async (req, res) => {
    ...
    jwt.sign({ agent }, process.env.JWT_KEY, { expiresIn: '1d' }, (err, token) => {
    ...
    try {
        jwt.verify(req.token, process.env.JWT_KEY, (err, authorizedData) => {
            if (err) {
                ...
                if (authorizedData.hasOwnProperty("agent")) { res.send(authorizedData); }
                else { res.send("Unauthorized access to agent's profile"); }
            }
        })
    } catch (e) {
        res.send(e);
